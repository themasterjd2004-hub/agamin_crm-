...
from django.db.models import Case, CharField, DecimalField, Exists, F, OuterRef, Subquery, Sum, Value, When
...
from crm.models import Currency, Output, Rate
from crm.utils.admfilters import PaymentByDepartmentFilter, ScrollRelatedOnlyFieldListFilter, CrmDateFieldListFilter
from crm.utils.helpers import get_counterparty_header

OFFICIAL_RATE = next((x[1] for x in Rate.RATE_TYPE if x[0] == Rate.OFFICIAL))
APPROXIMATE_RATE = next((x[1] for x in Rate.RATE_TYPE if x[0] == Rate.APPROXIMATE))
...
class PaymentAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        total_data = queryset.annotate(
            state_value=Case(
                When(
                    Exists(rate),
                    then=F('amount') * Subquery(rate.values('rate_to_state_currency')[:1])
                ),
            ...
            marketing_value=Case(
                When(
                    Exists(rate),
                    then=F('amount') * Subquery(rate.values('rate_to_marketing_currency')[:1])
                ),
                default=F('amount') * F('currency__rate_to_marketing_currency'),
                output_field=DecimalField(),
            ),
            rate_type=Case(
                When(
        ...
        response.context_data['vat_state_amount'] = round(sta * vatk, 2) if sta else 0

        mta = total_data['marketing_amount']
        response.context_data['marketing_amount'] = round(mta, 2) if mta else 0
        response.context_data['vat_marketing_amount'] = round(mta * vatk, 2) if mta else 0

        response.context_data['rate_type'] = OFFICIAL_RATE if total_data['type'] == 0 else APPROXIMATE_RATE

        # Handle cases where currency might not exist
        try:
            response.context_data['state_currency'] = Currency.objects.filter(
                is_state_currency=True
            ).first()
        except Currency.DoesNotExist:
            response.context_data['state_currency'] = None
        ...
        try:
            ...
            response.context_data['marketing_currency'] = None
    ...
    def get_list_filter(self, request):
        ...
        if request.user.is_superuser or request.user.is_chief or request.user.is_superoperator:
            if PaymentByDepartmentFilter not in list_filter:
    ...
    def coloured_payment_date(self, obj):
        ...
        return mark_safe(f'<span style="color: {color};">{date}</span>')
    ...
    def value(self, obj):
        if obj.amount and obj.through_representation:
            title = obj._meta.get_field('through_representation').verbose_name
...
def set_currency_initial(request, kwargs) -> None:
    try:
        if request.user.department_id:
            ...
            if department.default_currency_id:
                kwargs['initial'] = department.default_currency_id
                return

        # Fallback to state currency or first available currency
        currency = (Currency.objects.filter(is_state_currency=True).first() or
                    Currency.objects.first())
        if currency:
            kwargs['initial'] = currency.id
    except (ObjectDoesNotExist, AttributeError):
        # If all else fails, don't set an initial value
        pass